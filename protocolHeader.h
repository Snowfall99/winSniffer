...
typedef struct DHCP_Header
{
	u_char		op;						// 报文类型
	u_char		hw_type;				// 硬件类型
	u_char		hw_len;					// 硬件地址长度
	u_char		hops;					// 跳数
	u_int		xid;					// 事务ID
	u_short		secs;					// 客户启动时间
	u_short		flags;					// 标志
	IP_Address	client_ipaddr;			// 客户机IP地址
	IP_Address	you_ipaddr;				// 你的IP地址
	IP_Address	server_ipaddr;			// 服务器IP地址
	IP_Address	gate_ipaddr;			// 网关IP地址
	u_char		client_addr[16];		// 客户硬件地址
	u_char		server_name[64];		// 服务器主机名
